(function(global) {
    var CustomPagination = function(el, options) {
        ...
        if (options) { for (var key in options) { default_options[key] = options[key] } }
    ...
    CustomPagination.prototype = {
        init: function() {
            ...
            if (links && links.toLowerCase() === "yes") {
                prev_link_html = '<a class="prev-page" href="javascript:void(0)">上一页</a>';
                next_link_html = '<a class="next-page" href="javascript:void(0)">下一页</a>'
            }
            var inputs = this.options.inupt_forward,
                input_html = "";
            if (inputs && inputs.toLowerCase() === "yes") { input_html = '<div class="page-to">跳至<input type="text" id="page_input" />页<span class="go">GO</span></div>' }
            ...
            if (totals) {
                var page_li_html = "";
                if (totals > counts) {
                    for (var i = 1, len1 = counts; i < len1; i++) { if (i === 1) { page_li_html += '<li class="current-page">' + i + "</li>" } else { page_li_html += "<li>" + i + "</li>" } }
                    show_pages_html = "<ul>" + page_li_html + '<li class="ellipsis">…</li><li>' + totals + "</li>" + "</ul>"
                } else {
                    for (var j = 0, len2 = totals; j < len2; j++) { if (j === 0) { page_li_html += '<li class="current-page">' + (j + 1) + "</li>" } else { page_li_html += "<li>" + (j + 1) + "</li>" } }
            ...
            if (totals && counts) {
                ...
                this.bindClickPage();
                if (links && links.toLowerCase() === "yes") {
                    that.addEvent(this.el.firstChild.querySelector(".prev-page"), "click", function(e) { e = e.target ? e.target : e.srcElement; if (!e.hasAttribute("disabled")) { that.prevPage() } });
                    that.addEvent(this.el.firstChild.querySelector(".next-page"), "click", function(e) { e = e.target ? e.target : e.srcElement; if (!e.hasAttribute("disabled")) { that.nextPage() } })
                }
                if (inputs && inputs.toLowerCase() === "yes") { that.addEvent(this.el.firstChild.querySelector(".go"), "click", function(e) { that.searchInput() }) }
            }
            this.disablePreNext();
        ...
        nextPage: function() {
            ...
            if (this.options.total > this.options.count) {
                ...
                if (should_to_page > can_to_page) {
                    if (should_to_page === this.options.total) { var arr = currentx_select.parentNode.childNodes; var ellipsis = false; for (var i = 0; i < arr.length; i++) { if (arr[i].getAttribute("class") === "ellipsis") { ellipsis = true; break } } if (!ellipsis) { next_select.click(); return false } }
                    this.reGeneratePageBigger()
                } else { next_select.click() }
            } else { next_select.click() }
        },
        destroy: function() { this.el.innerHTML = "" },
                    ...
                    (function(l) {
                        that.addEvent(lis[l], "click", function(e) {
                            ...
                            if (new_page !== that.current_page) {
                                if (new_page === that.options.total) { var judgeEllipsis = li_event.previousElementSibling || li_event.previousSibling; if (judgeEllipsis.getAttribute("class") === "ellipsis") { that.clickEndPage(); return false } }
                                li_event.setAttribute("class", "current-page");
                                var siblings = that.siblings(li_event);
                                for (var k = 0; k < siblings.length; k++) { siblings[k].removeAttribute("class") }
        ...
        clickEndPage: function() {
            ...
            if (remainder === 1) {
                times -= 1;
                page_count = counts
            } else { if (remainder === 0) { page_count = counts - 1 } else { page_count = remainder } }
            var ul = this.el.firstChild.getElementsByTagName("ul")[0];
            ul.innerHTML = "";
            var page_li_html = "";
        ...
        searchInput: function() {
            ...
            if (input_val) {
                ...
                if (input_val < 0 || input_val === 0) { input_val = 1 } else { if (input_val > this.options.total) { input_val = this.options.total } }
                ...
                if (total > count) {
                    ...
                    if (total === input_val) { this.clickEndPage() } else {
                        ...
                        if (t_remainder === 0) { t_remainder = count - 1 }
                        var page_li_html = "";
                        if (t_times < all_times) {
                            if (t_times === c_times) { ul.childNodes[t_remainder - 1].click() } else {
                                var start2 = (t_times - 1) * (count - 1);
                                var end2 = t_times * (count - 1);
                                for (var i = start2 + 1, len1 = end2 + 1; i < len1; i++) { page_li_html += "<li>" + i + "</li>" }
                ...
                } else { if (this.current_page !== input_val) { ul.childNodes[input_val - 1].click() } }
                input.value = ""
            }
        },
        addEvent: function(elem, type, fn) { if (elem.attachEvent) { elem.attachEvent("on" + type, fn); return } if (elem.addEventListener) { elem.addEventListener(type, fn, false) } },
        siblings: function(currentNode) { var siblings = []; var elseLi = currentNode.parentNode.children; for (var i = 0, elseLil = elseLi.length; i < elseLil; i++) { if (elseLi[i] !== currentNode && elseLi[i].getAttribute("class") !== "ellipsis") { siblings.push(elseLi[i]) } } return siblings }
    };
    if (typeof module !== "undefined" && module.exports) { module.exports = CustomPagination }
    if (typeof define === "function") { define(function() { return CustomPagination }) }
