...
export function export_json_to_excel({
  ...
  arrabc.some(function(v) {
    debugger
    for (let j = 1; j < multiHeader.length + 4; j++) {
      const _v = v + j
      if (dataInfo[_v]) {
        dataInfo[_v].s = {}
        // 标题部分A1-Z1
        ...
        // 表头部分,根据表头特殊格式设置
          ...
          // multiHeader.length = 0 时表头没有合并单元格，表头只占1行A2-Z2
            const fv = v + (multiHeader.length + 3)
            dataInfo[fv].s = {
              border: borderAll,
              font: {
                name: '微软雅黑',
        ...
        // multiHeader.length + 3 是表头的最后1行
        dataInfo[v + (multiHeader.length + 3)].s = {
          border: borderAll,
          font: {
            name: '微软雅黑',
            sz: 10
