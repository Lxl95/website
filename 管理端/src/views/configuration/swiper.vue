        ...
        <el-table-column label="图片" align="left">
          <template slot-scope="scope">
            <!-- <el-image
              ...
              :src="imgBaseUrl + scope.row.urlpath"
              fit="cover"
            /> -->
            <vviewer
              :options="scope.row.urlpath?[imgBaseUrl + scope.row.urlpath]:[]"
              :imgStyle="'contain'"
              style="width: 140px; height: 70px;"
            ></vviewer>
          </template>
...
export default {
  ...
  computed: {},
  mounted() {
    console.log(this.$store.getters.jsonInfo.imageUrl);
    this.tableHeight =
      window.innerHeight -
  ...
  methods: {
    ...
    getList() {
      getImgList().then(res => {
        ...
        this.$nextTick(() => {
          this.$refs.loopImgTable.doLayout();
        })
      });
    },
    // 添加轮播图片
