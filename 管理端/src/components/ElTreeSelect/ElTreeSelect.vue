<template>
  <div class="el-tree-select" :class="selectClass">
    <!-- 下拉文本 -->
    <el-select ref="select" v-model="labels" v-popover:popover :style="styles" class="el-tree-select-input"
      :disabled="disabled" popper-class="select-option" v-bind="selectParams" :popper-append-to-body="false"
      :filterable="selectParams.filterable" :filter-method="filterFun" :multiple="selectParams.multiple" :title="labels"
      @remove-tag="_selectRemoveTag" @clear="_selectClearFun" @focus="_popoverShowFun" />
    <!-- 弹出框 -->
    <el-popover ref="popover" v-model="visible" :placement="placement" :popper-class="popperClass" :width="width"
      trigger="click">
      <!-- 是否显示搜索框 -->
      ...
      <el-scrollbar tag="div" wrap-class="el-select-dropdown__wrap" view-class="el-select-dropdown__list"
        class="is-empty">
        <!-- 树列表 -->
        <el-tree v-show="data.length > 0" ref="tree" v-bind="treeParams" :data="data" :node-key="propsValue"
          :draggable="false" :current-node-key="ids.length > 0 ? ids[0] : ''" :show-checkbox="selectParams.multiple"
          :filter-node-method="_filterFun" :render-content="treeRenderFun" @node-click="_treeNodeClickFun"
          @check="_treeCheckFun" />
        <!-- 暂无数据 -->
        ...
        if (el.getCurrentNode()) {
          if (this.propsCode) {
            ...
            this.labels = el.getCurrentNode()[this.propsCode]
              ? el.getCurrentNode()[this.propsLabel] + '(' + el.getCurrentNode()[this.propsCode] + ')'
              : el.getCurrentNode()[this.propsLabel]
          } else {
            if (typeof this.propsLabel === 'function') {
              this.labels = el.getCurrentNode().meta.title
            } else {
