    ...
    jkApp.run(['$rootScope', '$location', function($rootScope, $location) {
        /* 监听路由的状态变化 */
        $rootScope.$on('$routeChangeStart', function(evt, next, current){
          ...
          if(!getlocalStorage('openId') && $rootScope.pageName == 'business' && next_path !== '/OnlineBusinessHall' ) {
            $location.path('/bindWeChat')
          }
        }); 
        $rootScope.$on('$routeChangeSuccess', function(evt, current, previous) {
          console.log('route have already changed ：'+ current.$$route.originalPath );
        }); 
    }])
    // //创建一个ajax服务
    ...
    jkApp.controller('loginCtrl', function($scope, $location) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    $scope.$apply(function() {
                        ...
                        $("#codeImg").attr('src', "data:img/png;base64," + window.btoa(binary));
                        $(".codeImg2").attr('src', "data:img/png;base64," + window.btoa(binary));
                    });
                },
        ...
        $scope.sendMessage = function () {
            ...
            //向后台发送处理数据
            $.ajax({
                url: baseUrl + `/registration/sendInformation?phoneNumber=`+phone+`&code=`+$("#code4Sms").val()+'&codeId='+$scope.Codeid,
                ...
                success : function(result) {
                    ...
                    InterValObj = window.setInterval(function(){SetRemainTime("btnSendCode")}, 1000); //启动计时器，1秒执行一次
                    curCount = count;
                    //设置button效果，开始计时
        ...
        function checkQRCode(type) {
            $.ajax({
                type : "GET",
                url : type == 'wx' ? "sso/checkWxQRCode?v="+Math.random()
    ...
    jkApp.controller('registerCtrl', function($scope, $location) {
        ...
        $scope.sendMessage = function () {
            ...
            //向后台发送处理数据
            $.ajax({
                url: baseUrl + `/registration/sendInformation?phoneNumber=`+phone+`&code=`+$("#code").val()+'&codeId='+$scope.Codeid,
    ...
    jkApp.controller('businessCtrl', function($scope, $location) {
        ...
        function getConsDetail() {
            ...
            $.ajax({
                type: 'GET',
                url: baseUrl_API_WeiXin + 'netUserInfoController/findAllUser?userInfoCode=' + $scope.lastOperatedTable,
                success: function (res) {
                if (res.code == 1) {
                    $scope.$apply(function() {
                        $scope.consInfo =  res;
        ...
        function getMoneyDetail() {
            ...
            $.ajax({
                type: 'GET',
                url: baseUrl_API_WeiXin + 'netUserInfoController/netFindOweBillDetails?userInfoCode=' + $scope.lastOperatedTable,
                success: function (res) {
                if (res.code == 0) {
                    $scope.$apply(function() {
    ...
    jkApp.controller('accountCtrl', function($scope, $location) {
        ...
        function changeCons(lastOperatedTable) {
            if(lastOperatedTable) {
                $.ajax({
                    type: 'GET',
                    url: baseUrl_API_WeiXin + 'payUser/updateLastByOpenId?openId=' + getlocalStorage('openId') +'&lastOperatedTable=' + lastOperatedTable,
                    success: function (res) {
                        if (res.code == 10000) {
        ...
        function tableInit() {
            layui.use(['form', 'layedit', 'laydate', 'table'], function() {
                ...
                var table_name = table.render({
                    ...
                    where: {
                        pagination: JSON.stringify({"page":1,"rows":200,"sidx":"RCVBL_YM","sord":"desc"}),
                ...
                $scope.searchQuery = function() {
                    ...
                    table.reload('testReload', {
                        ...
                        where: {
                            pagination: JSON.stringify({"page":1,"rows":100,"sidx":"RCVBL_YM","sord":"desc"}),
    ...
    jkApp.controller('paymentQueryCtrl', function($scope, $location) {
        ...
        function tableInit() {
            ...
            var table_name = table.render({
                ...
                where: {
                    pagination: JSON.stringify({"page":1,"rows":200,"sidx":"","sord":"desc"}),
            ...
            $scope.searchQuery = function() {
                ...
                table.reload('testReload', {
                    ...
                    where: {
                        pagination: JSON.stringify({"page":1,"rows":200,"sidx":"","sord":"desc"}),
                        queryJson: JSON.stringify({"UserInfoCode": $scope.myConsNo || $scope.lastOperatedTable})
                    },
                    parseData: function(res) { //res 即为原始返回的数据
                        return {
                            "code": res.code == 200 ? 0 : '', //解析接口状态
        ...
        function getFileMsg() {
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    Entity: {
                      FlowId:
                        liuchengMes.peopleNewInstallLC.FlowId,
                      NodeId:
                        liuchengMes.peopleNewInstallLC.NodeId,
                    },
                }),
                dataType: 'JSON',
                success: function(res) {
        ...
        function getFileMsg() {
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    Entity: {
                      FlowId:
                        liuchengMes.chooseUserNameLC.FlowId,
                      NodeId:
                        liuchengMes.chooseUserNameLC.NodeId,
                    },
                }),
                dataType: 'JSON',
                success: function(res) {
        ...
        function getFileMsg() {
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    Entity: {
                      FlowId:
                        liuchengMes.cancelUserLC.FlowId,
                      NodeId:
                        liuchengMes.cancelUserLC.NodeId,
                    },
                }),
                dataType: 'JSON',
                success: function(res) {
        ...
        function getFileMsg() {
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    Entity: {
                      FlowId:
                        liuchengMes.NatureOfWaterChange.FlowId,
                      NodeId:
                        liuchengMes.NatureOfWaterChange.NodeId,
                    },
                }),
                dataType: 'JSON',
                success: function(res) {
            ...
            var table_name = table.render({
                ...
                cols: [
                    [ //表头
                        ...
                        { field: 'CreateTime', title: '上报时间', align: "center", templet: function(d){
                                ...
                                success: function () {
                                    layui.table.render({
                                        ...
                                        cols: [
                                            [ //表头
                                                { field: 'InsName', title: '项目名称', align: "center" },
                                                { field: 'FileRlName', title: '上传附件', align: "center" },
                                                { field: 'FileName', title: '文件名称', align: "center" },
                                                { field: 'CreateTime', title: '上报时间', align: "center", templet: function(d){
                                                    return '<span>'+ d.CreateTime.replace('T', ' ') +'</span>'
                                                } },
                                                { field: 'NodeName', title: '当前进度', align: "center" },
                                                { field: '', title: '查看', align: "center", toolbar: "#ImgDemo"},
                                            ]
                                        ]
                                    })
